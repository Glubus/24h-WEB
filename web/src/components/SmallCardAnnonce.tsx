...
export function SmallCardAnnonce({
  ...
  const imageUrl = images.length > 0 ? annonceImageUrl(id) : DEFAULT_IMAGE;
  const sellerName =
    typeof author === "string" ? "Vendeur" : formatUsername(author.username);
  const sellerRating = typeof author === "string" ? null : author.rating;
  const sellerId = typeof author === "string" ? idFromIri(author) : author.id;
  const sellerImageUrl =
    sellerId === undefined ? null : userPictureUrl(sellerId);
  const sellerInitial = sellerName.charAt(0).toUpperCase();
  const createdAtLabel = formatDate(createdAt);
  const isFavorite =
    currentUserId === undefined
      ? false
      : relationContainsUser(favorites, currentUserId);
    ...
    >
      <figure>
        <div className={"relative"}>
          ...
          <div className={"badge absolute top-2 left-2 badge-ghost badge-lg"}>
            {price}€
          </div>
          {isFavorite ? (
      ...
      <div className="card-body p-3 text-left">
        ...
        <div className="mt-1 flex flex-col gap-1 text-sm text-base-content/70">
          ...
          <p className="flex items-center gap-2">
            <span
              className={`avatar ${sellerImageUrl === null ? "placeholder" : ""}`}
            >
              <div className="h-6 w-6 rounded-full bg-primary text-xs text-primary-content">
                {sellerImageUrl === null ? (
...
function relationContainsUser(
  users: AnnonceListItem["favorites"],
  userId: number,
) {
  return (
    users?.some((user) => {
