...
export class ApiPlatformClient {
  ...
  constructor(options: ApiPlatformClientOptions = {}) {
    this.baseUrl = normalizeBaseUrl(
      options.baseUrl ?? import.meta.env.VITE_API_URL ?? "/api",
    );
  ...
  listAnnonces(filters: AnnonceFilters = {}) {
    return this.get<ApiCollection<AnnonceListItem>>(
      `/annonces${this.toAnnonceQuery(filters)}`,
      { authenticated: false },
    );
  }
  ...
  listAnnonceCategories() {
    return this.get<ApiCollection<AnnonceCategoryResource>>(
      "/annonces/categories",
      { authenticated: false },
    );
  }
  ...
  listMessages(conversationId: ApiId) {
    return this.get<ApiCollection<Message>>(
      `/messages?conversation=/api/conversations/${conversationId}`,
    );
  }
