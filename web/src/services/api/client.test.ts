...
describe("ApiPlatformClient", () => {
  it("logs in with JSON credentials and stores the returned token", async () => {
    ...
    fetchMock.mockResolvedValueOnce(
      jsonResponse({ token: "token-123", username: "admin" }),
    );
  ...
  it("lists annonces with Api Platform search and range filters", async () => {
    const client = new ApiPlatformClient({ baseUrl: apiBaseUrl });
    fetchMock.mockResolvedValueOnce(
      jsonResponse({ member: [], totalItems: 0 }),
    );
  ...
  it("patches annonces with merge-patch JSON and bearer token", async () => {
    ...
    fetchMock.mockResolvedValueOnce(
      jsonResponse({ id: 7, title: "Nouveau titre" }),
    );

    await client.updateAnnonce(7, {
      title: "Nouveau titre",
      city: "Montpellier",
    });
    const [url, init] = lastFetchCall();
    ...
    expect(init?.body).toBe(
      JSON.stringify({ title: "Nouveau titre", city: "Montpellier" }),
    );
  ...
  it("uploads annonce images as multipart form data without forcing a content type", async () => {
    ...
    const image = new File(["image-content"], "annonce.png", {
      type: "image/png",
    });
    fetchMock.mockResolvedValueOnce(
      jsonResponse({ id: 7, images: ["/uploads/annonce.png"] }),
    );
  ...
  it("uploads user profile pictures through the API pictures route", async () => {
    ...
    fetchMock.mockResolvedValueOnce(
      jsonResponse({ id: 3, profileImagePath: "/uploads/users/profile.png" }),
    );
  ...
  it("gets public user summaries without sending bearer token", async () => {
    ...
    fetchMock.mockResolvedValueOnce(
      jsonResponse({ id: 5, username: "seller", rating: 4.5 }),
    );
