...
export function AnnoncePage({ currentUser, onNavigate, onNavigateChat, annonceId }: AnnoncePageProps) {
  ...
  const imageUrls = annonce.images.length > 0
    ? annonce.images.map((_, index) => annonceImageUrl(annonce.id, index))
    : [fallbackImageUrl]
  const selectedImageIndex = selectedImage.annonceId === annonce.id ? selectedImage.index : 0
  const imageUrl = imageUrls[selectedImageIndex] ?? imageUrls[0]
  const displayAnnonce = favoriteOverride?.id === annonce.id ? favoriteOverride : annonce
  const favoriteCount = displayAnnonce.favoriteCount ?? displayAnnonce.favorites?.length ?? 0
  const coordinates = annonce.latitude && annonce.longitude
    ? [{
        id: annonce.id,
  ...
  const isFavorite = currentUser?.id === undefined ? false : relationContainsUser(displayAnnonce.favorites, currentUser.id)
  ...
  async function handleStartChat() {
    ...
    try {
      ...
      setChatError(startError instanceof Error ? startError.message : 'Conversation impossible.')
    } finally {
  ...
  async function handleRateSeller() {
    ...
    try {
      const updatedAnnonce = await api.rateAnnonceSeller(currentAnnonce.id, ratingValue)
      setSellerRatingOverride(ratingValue)
      setFavoriteOverride(updatedAnnonce)
    } catch (rateError) {
      setRatingError(rateError instanceof Error ? rateError.message : 'Note impossible.')
    } finally {
          ...
          <div className="flex gap-6 mt-10">
              ...
              <div className="flex h-[650px] w-[500px] shrink-0 flex-col">
                  ...
                  >
	                  ...
	                  >
                      {isStartingChat ? <span className="loading loading-spinner loading-sm" /> : null}
                  ...
                  {!isAuthor && currentUser !== null ? (
                      <div className="mt-4 rounded-lg border border-base-300 bg-base-100 p-4">
                          ...
                          <div className="mt-3 flex items-center gap-3">
                              ...
                              >
                                  {isSavingRating ? <span className="loading loading-spinner loading-xs" /> : null}
                                  Valider
                              </button>
                          </div>
