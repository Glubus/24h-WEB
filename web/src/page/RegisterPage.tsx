    ...
    <main className="flex justify-center py-12">
      <div className="card mt-6 w-full max-w-md border border-base-300 bg-base-100 shadow-xl">
        <form className="card-body gap-4" onSubmit={handleSubmit}>
          ...
          <button type="button" className="btn btn-outline w-full" onClick={() => onNavigate('login')}>
            Déjà inscrit ? Se connecter
          </button>
          <button type="button" className="btn btn-ghost w-full" onClick={() => onNavigate('home')}>
            Retour
          </button>
        </form>
      </div>
...
function MailIcon() {
  return (
    <svg className="h-4 w-4 opacity-70" fill="currentColor" viewBox="0 0 16 16" aria-hidden="true">
