...
export function ChatPage({
  ...
  const [annonces, setAnnonces] = useState<AnnonceListItem[]>([]);
  const [content, setContent] = useState("");
  const [linkedAnnonce, setLinkedAnnonce] = useState<AnnonceListItem | null>(
    null,
  );
  const [error, setError] = useState<string | null>(null);
  ...
  const selectedOtherUser =
    selectedConversation === null || currentUser === null
      ? null
      : otherParticipant(selectedConversation, currentUser);
  const linkableAnnonces =
    selectedConversation === null
      ...
      : annonces.filter((annonce) => {
          ...
          return (
            id === selectedConversation.userOne.id ||
            id === selectedConversation.userTwo.id
          );
        });
  ...
  async function handleSendMessage(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const message = await api.createMessage({
        ...
        annonce:
          linkedAnnonce === null ? null : `/api/annonces/${linkedAnnonce.id}`,
      });
      ...
      setError(
        sendError instanceof Error ? sendError.message : "Message impossible.",
      );
    }
  }
    ...
    setMessages((currentMessages) =>
      currentMessages.map((message) =>
        message.id === messageId
          ? { ...message, content: "Message supprimé", deleted: true }
          : message,
      ),
    );
  }
    ...
    <main className="py-8">
      <div className="grid min-h-[650px] gap-5 lg:grid-cols-[320px_1fr]">
        <aside className="rounded-lg border border-base-300 bg-base-100 p-4">
          <h2 className="px-2 text-sm font-semibold uppercase text-base-content/60">
            Conversations
          </h2>
          <div className="mt-3 flex flex-col gap-2">
            {conversations.map((conversation) => {
              ...
              return (
                ...
                >
                  <span
                    className={`avatar shrink-0 ${avatarUrl === null ? "placeholder" : ""}`}
                  >
                    <div className="h-9 w-9 overflow-hidden rounded-full bg-primary text-primary-content">
                      {avatarUrl === null ? (
                        <span>{initial}</span>
                      ) : (
                        <img src={avatarUrl} alt={otherUser.username} />
                      )}
                    </div>
                  </span>
                  <span className="min-w-0 text-left">
                    <span className="flex min-w-0 items-center gap-2">
                      <span className="truncate">
                        {formatUsername(otherUser.username)}
                      </span>
                      <time
                        className="ml-auto shrink-0 text-xs opacity-60"
                    ...
                    <span className="block text-xs opacity-70">
                      {otherUser.successfulSaleCount ?? 0} ventes réussies · ★{" "}
                      {formatRating(otherUser.rating)}
                    </span>
                  </span>
                </button>
              );
          ...
          <header className="border-b border-base-300 p-4">
            {selectedOtherUser === null ? (
              ...
              <div className="flex items-center gap-3">
                <div
                  className={`avatar ${selectedOtherUser.id === undefined ? "placeholder" : ""}`}
                >
                  <div className="h-12 w-12 overflow-hidden rounded-full bg-primary text-primary-content">
                    {selectedOtherUser.id === undefined ? (
                      <span>
                        {selectedOtherUser.username.charAt(0).toUpperCase()}
                      </span>
                    ) : (
                      <img
                        src={userPictureUrl(selectedOtherUser.id)}
                ...
                <div>
                  <h1 className="text-xl font-bold">
                    {formatUsername(selectedOtherUser.username)}
                  </h1>
                  <p className="text-sm text-base-content/60">
                    {selectedOtherUser.successfulSaleCount ?? 0} ventes réussies
                    · ★ {formatRating(selectedOtherUser.rating)}
                  </p>
                </div>
              </div>
            )}
              ...
              return (
                ...
                >
                    ...
                    typeof message.annonce === "string" ? null : (
                      <div className="mt-3 flex gap-3 rounded bg-base-100/20 p-2 text-sm">
                        <img
                          className="h-14 w-16 rounded object-cover"
                          src={
                            message.annonce.images.length > 0
                              ? annonceImageUrl(message.annonce.id)
                              : undefined
                          }
                  ...
                  {message.deleted ? null : (
                    <button
                      type="button"
                      className="chat-footer text-xs opacity-70"
            ...
            <div className="dropdown dropdown-top dropdown-end">
              ...
              <div className="dropdown-content z-[1] max-h-80 w-80 overflow-y-auto rounded-box border border-base-300 bg-base-100 p-2 shadow">
                {linkedAnnonce === null ? null : (
                  <button
                    type="button"
                    className="btn btn-ghost btn-sm mb-2 w-full justify-start"
                    onClick={() => setLinkedAnnonce(null)}
                  >
                    Retirer l'annonce
                  </button>
                )}
...
function otherParticipant(conversation: Conversation, currentUser: User) {
  return conversation.userOne.id === currentUser.id
